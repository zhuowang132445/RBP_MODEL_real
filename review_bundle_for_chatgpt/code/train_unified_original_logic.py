...
def enable_motif_non_esm(model):
...
def enable_binding_backbone(model, train_binding_protein_embedding: bool):
    for name, p in model.binding_model.named_parameters():
        if name.startswith("protein_embedding.") and not train_binding_protein_embedding:
...
def set_stage_trainability(
    model,
    stage,
    train_binding_backbone: bool = False,
    train_binding_protein_embedding: bool = False,
    train_motif_non_esm: bool = False,
):
    for p in model.parameters():
        p.requires_grad = False
    if stage == "motif":
        enable_motif_non_esm(model)
    if stage in {"binding", "joint", "late_fusion"}:
        if train_binding_backbone:
            enable_binding_backbone(model, train_binding_protein_embedding=train_binding_protein_embedding)
        elif train_binding_protein_embedding:
            for name, p in model.binding_model.named_parameters():
                if name.startswith("protein_embedding."):
                    p.requires_grad = True
        for p in model.motif_binding_projector.parameters():
            p.requires_grad = True
        for p in model.motif_aware_classifier.parameters():
            p.requires_grad = True
    if stage == "joint" and train_motif_non_esm:
        enable_motif_non_esm(model)


def parameter_module_name(parameter_name: str) -> str:
    parts = parameter_name.split(".")
    if parts[0] in {"binding_model", "motif_model"} and len(parts) >= 2:
        return f"{parts[0]}.{parts[1]}"
    return parts[0]


def build_trainable_parameter_report(model) -> pd.DataFrame:
    rows = []
    total_by_module = defaultdict(int)
    trainable_by_module = defaultdict(int)
    for name, param in model.named_parameters():
        module_name = parameter_module_name(name)
        numel = int(param.numel())
        total_by_module[module_name] += numel
        if param.requires_grad:
            trainable_by_module[module_name] += numel
    for module_name in sorted(total_by_module):
        total = int(total_by_module[module_name])
        trainable = int(trainable_by_module.get(module_name, 0))
        rows.append(
            {
                "module_name": module_name,
                "total_parameters": total,
                "trainable_parameters": trainable,
                "trainable_fraction": float(trainable / total) if total else float("nan"),
            }
        )
...
def main():
    ...
    ap.add_argument("--train-binding-backbone", action="store_true")
    ap.add_argument("--train-binding-protein-embedding", action="store_true")
    ap.add_argument("--train-motif-non-esm", action="store_true")
    ...
    set_stage_trainability(
        model,
        args.stage,
        train_binding_backbone=bool(args.train_binding_backbone),
        train_binding_protein_embedding=bool(args.train_binding_protein_embedding),
        train_motif_non_esm=bool(args.train_motif_non_esm),
    )
    trainable_parameter_report = build_trainable_parameter_report(model)
    trainable_parameter_report.to_csv(out / "trainable_parameter_report.tsv", sep="\t", index=False)
    ...
    if args.dry_run_check:
        ...
        report = {
            ...
            "trainable_parameter_report_tsv": str(out / "trainable_parameter_report.tsv"),
    ...
    report = {
        ...
        "stage": args.stage,
        "checkpoint": str(ckpt),
        "last_metrics": hist[-1] if hist else {},
        "train_binding_backbone": bool(args.train_binding_backbone),
        "train_binding_protein_embedding": bool(args.train_binding_protein_embedding),
        "train_motif_non_esm": bool(args.train_motif_non_esm),
        "trainable_parameter_report_tsv": str(out / "trainable_parameter_report.tsv"),
