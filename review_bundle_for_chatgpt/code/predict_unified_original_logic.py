...
FUNGI_SPECIES = {"YEAST", "SCHPO", "CANAL", "ASPOR", "NEUCR"}
ANIMAL_SPECIES = {"HUMAN", "MOUSE", "DROME", "CAEEL", "DANRE", "XENLA"}
...
def kingdom_label_from_query_id(query_protein_id: str) -> int:
    ...
    if prefix in PLANT_SPECIES:
        return 1
    if prefix in FUNGI_SPECIES:
        return 2
    if prefix in ANIMAL_SPECIES:
        return 0
    return 3


def resolve_query_kingdom(query_protein_id: str, fallback_label: int = 1) -> Dict[str, object]:
    inferred = kingdom_label_from_query_id(query_protein_id)
    if inferred != 3:
        return {
            "label": int(inferred),
            "name": KINGDOM_LABEL_TO_NAME[int(inferred)],
            "source": "query_protein_id_prefix",
        }
    fallback = int(fallback_label)
    return {
        "label": fallback,
        "name": KINGDOM_LABEL_TO_NAME.get(fallback, "other"),
        "source": "fallback_label",
    }


def normalize_kmer_vocab(kmers: np.ndarray) -> np.ndarray:
    return np.asarray([str(k).strip().upper().replace("T", "U") for k in kmers.astype(str).tolist()], dtype=object)
...
def safe_sigmoid(values: np.ndarray) -> np.ndarray:
    ...
    if finite.any():
        out[finite] = 1.0 / (1.0 + np.exp(-arr[finite]))
    return out
...
def build_motif_outputs(
    ...
    for rbp_id in query_ids:
        ...
        seq = clean_protein(seq)
        kingdom_info = resolve_query_kingdom(query_protein_id, fallback_label=fallback_kingdom_label)
        sequences.append(seq)
        phys.append(phys_feats(seq))
        labels.append(int(kingdom_info["label"]))
...
def score_windows_for_rbp(
    ...
    out["base_score"] = safe_sigmoid(out["base_logit"].to_numpy(dtype=np.float32))
    out["inverted_base_score"] = np.where(np.isfinite(out["base_score"]), 1.0 - out["base_score"], np.nan)
    if use_joint_head:
        out["joint_logit"] = np.concatenate(joint_logits_all)
        out["joint_score"] = safe_sigmoid(out["joint_logit"].to_numpy(dtype=np.float32))
...
def build_kmer_index(kmers: np.ndarray) -> Dict[str, int]:
    return {str(kmer).strip().upper().replace("T", "U"): idx for idx, kmer in enumerate(kmers.astype(str).tolist())}
...
def robust_center_scale(raw: np.ndarray) -> Tuple[np.ndarray, float, float]:
    ...
    if not finite.any():
        return np.full(raw.shape, np.nan, dtype=np.float32), float("nan"), float("nan")
    observed = raw[finite]
    ...
    if not np.isfinite(scale) or scale < 1e-6:
        scale = 1.0
    score = np.full(raw.shape, np.nan, dtype=np.float32)
    score[finite] = (raw[finite] - center) / scale
    return score.astype(np.float32), center, scale


def add_score_ablation_columns(
    frame: pd.DataFrame,
    motif_alpha: float,
    structure_alpha: float,
) -> pd.DataFrame:
    out = frame.copy()
    base_logit = out["base_logit"].to_numpy(dtype=np.float32)
    out["inverted_base_score"] = np.where(np.isfinite(out["base_score"]), 1.0 - out["base_score"], np.nan)
    motif_score = out["motif_match_score"].to_numpy(dtype=np.float32) if "motif_match_score" in out.columns else np.full(len(out), np.nan, dtype=np.float32)
    structure_score = out["structure_match_score"].to_numpy(dtype=np.float32) if "structure_match_score" in out.columns else np.full(len(out), np.nan, dtype=np.float32)
    motif_logit = np.where(np.isfinite(motif_score), float(motif_alpha) * motif_score, np.nan)
    structure_logit = np.where(np.isfinite(structure_score), float(structure_alpha) * structure_score, np.nan)

    out["motif_only_score"] = safe_sigmoid(motif_logit)
    out["structure_only_score"] = safe_sigmoid(structure_logit)
    out["motif_structure_score"] = safe_sigmoid(motif_logit + structure_logit)
    out["base_motif_score"] = safe_sigmoid(base_logit + motif_logit)
    out["inverted_base_motif_score"] = safe_sigmoid(-base_logit + motif_logit)
    out["base_motif_structure_score"] = safe_sigmoid(base_logit + motif_logit + structure_logit)
    out["inverted_base_motif_structure_score"] = safe_sigmoid(-base_logit + motif_logit + structure_logit)
    return out
...
def add_posthoc_motif_scores(
    ...
    out["posthoc_score"] = safe_sigmoid(out["posthoc_logit"].to_numpy(dtype=np.float32))
    out["score_delta_posthoc_minus_base"] = out["posthoc_score"] - out["base_score"]
    out = add_score_ablation_columns(
        frame=out,
...
def finalize_gene_rows(
    ...
    for gene_id, group in frame.groupby("gene_id", sort=False):
        ...
        score_values = g[score_col].to_numpy(dtype=float)
        ...
        if len(finite_scores) == 0:
            ...
            top5_percent_mean = float("nan")
            window_count_adjusted = float("nan")
        else:
            top3_mean = float(np.mean(finite_scores[: min(3, len(finite_scores))]))
            top5_mean = float(np.mean(finite_scores[: min(5, len(finite_scores))]))
            top10_mean = float(np.mean(finite_scores[: min(10, len(finite_scores))]))
            top5_percent_n = max(1, int(math.ceil(len(finite_scores) * 0.05)))
            top5_percent_mean = float(np.mean(finite_scores[:top5_percent_n]))
            max_score = float(finite_scores[0])
            window_count_adjusted = float(max_score - float(window_count_penalty) * math.log2(len(g) + 1.0))
        best = g.iloc[0]
        rows.append(
            {
                "rbp_id": rbp_id,
                ...
                "top5_percent_mean_score": top5_percent_mean,
                "window_count_adjusted_score": window_count_adjusted,
                "best_window_start": int(best["window_start"]),
                "best_window_end": int(best["window_end"]),
                "best_window_seq": str(best["rna_seq"]),
                "primary_window_score_name": score_col,
...
def compute_binary_metrics(labels: np.ndarray, scores: np.ndarray) -> Dict[str, float]:
    ...
    return {
        ...
        "positive_mean": float(np.mean(pos_scores)) if len(pos_scores) else float("nan"),
        "negative_mean": float(np.mean(neg_scores)) if len(neg_scores) else float("nan"),
...
def summarize_labeled_predictions(rbp_id: str, frame: pd.DataFrame) -> Dict[str, float | str]:
    ...
    row = {
        ...
        "base_positive_mean": base_metrics["positive_mean"],
        "base_negative_mean": base_metrics["negative_mean"],
        ...
        row.update(
            {
                ...
                "joint_positive_mean": joint_metrics["positive_mean"],
                "joint_negative_mean": joint_metrics["negative_mean"],
        ...
        row.update(
            {
                ...
                "joint_positive_mean": float("nan"),
                "joint_negative_mean": float("nan"),
        ...
        row.update(
            {
                ...
                "posthoc_positive_mean": posthoc_metrics["positive_mean"],
                "posthoc_negative_mean": posthoc_metrics["negative_mean"],
        ...
        row.update(
            {
                ...
                "posthoc_positive_mean": float("nan"),
                "posthoc_negative_mean": float("nan"),
...
GENE_RANKING_COLUMNS = {
    "max": "max_score",
    "top3_mean": "top3_mean_score",
    "top5_mean": "top5_mean_score",
    "top10_mean": "top10_mean_score",
    "top5_percent_mean": "top5_percent_mean_score",
    "window_count_adjusted": "window_count_adjusted_score",
}


def build_gene_ranking_views(gene_scores: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    views: Dict[str, pd.DataFrame] = {}
    for ranking_name, score_col in GENE_RANKING_COLUMNS.items():
        frame = gene_scores.copy()
        frame["ranking_name"] = ranking_name
        frame["ranking_score"] = frame[score_col]
        frame = frame.sort_values(
            ["rbp_id", "ranking_score", "gene_id"],
            ascending=[True, False, True],
            na_position="last",
        ).reset_index(drop=True)
        frame["rank"] = frame.groupby("rbp_id").cumcount() + 1
        views[ranking_name] = frame
    return views


def compute_gene_ranking_correlations(gene_scores: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for ranking_name, score_col in GENE_RANKING_COLUMNS.items():
        for rbp_id, sub in gene_scores.groupby("rbp_id", sort=False):
            pair = sub[["n_windows", score_col]].dropna()
            if len(pair) >= 2:
                pearson = float(pair["n_windows"].corr(pair[score_col], method="pearson"))
                spearman = float(pair["n_windows"].corr(pair[score_col], method="spearman"))
            else:
                pearson = float("nan")
                spearman = float("nan")
            rows.append(
                {
                    ...
                    "n_genes": int(len(sub)),
                    "pearson_n_windows_vs_score": pearson,
                    "spearman_n_windows_vs_score": spearman,
                }
            )
    return pd.DataFrame(rows)


def summarize_window_score_ablation(rbp_id: str, frame: pd.DataFrame) -> pd.DataFrame:
    labels = frame["label"].to_numpy(dtype=int)
    rows = []
    for score_name in [
        "base_score",
        ...
        if score_name not in frame.columns:
            continue
        metrics = compute_binary_metrics(labels, frame[score_name].to_numpy(dtype=float))
        rows.append(
            {
                "rbp_id": rbp_id,
                ...
                "positive_median": metrics["positive_median"],
                "negative_median": metrics["negative_median"],
            }
        )
    return pd.DataFrame(rows)
...
def parse_args():
    ...
    parser.add_argument("--auto-teacher-query-kingdom-label", type=int, default=1)
...
def main():
    ...
    resolved_query_kingdoms = {}
    for rbp_id in query_ids:
        query_protein_id = cfg.rbp_id_to_query_protein_id[rbp_id]
        resolved_query_kingdoms[rbp_id] = resolve_query_kingdom(
            query_protein_id,
            fallback_label=args.auto_teacher_query_kingdom_label,
        )
    ...
    kmers = normalize_kmer_vocab(np.load(motif_profile_npz)["kmers"].astype(str))
    if args.motif_profile_mode == "override":
        ...
        motif_profiles, teacher_summary, teacher_selected, teacher_conflicts = build_auto_teacher_outputs(
            ...
            query_kingdom_labels=[int(resolved_query_kingdoms[rbp_id]["label"]) for rbp_id in query_ids],
        ...
        motif_top, motif_summary, motif_profiles = build_motif_outputs(
            ...
            top_k=args.top_kmers,
            device=device,
            fallback_kingdom_label=args.auto_teacher_query_kingdom_label,
        )
    ...
    report: Dict[str, object] = {
        ...
        "resolved_query_kingdoms": {
            rbp_id: {
                "resolved_kingdom_label": int(info["label"]),
                "resolved_kingdom_name": str(info["name"]),
                "resolved_kingdom_source": str(info["source"]),
            }
            for rbp_id, info in resolved_query_kingdoms.items()
        },
        ...
        "window_count_adjusted_formula": "max_score - window_count_penalty * log2(n_windows + 1)",
    ...
    if args.window_tsv:
        ...
        all_window_rows = []
        all_gene_rows = []
        all_gene_views: Dict[str, List[pd.DataFrame]] = {name: [] for name in GENE_RANKING_COLUMNS}
        eval_rows = []
        ablation_rows = []
        for rbp_id in query_ids:
            query_protein_id = cfg.rbp_id_to_query_protein_id[rbp_id]
            ...
            scored["resolved_kingdom_label"] = int(resolved_query_kingdoms[rbp_id]["label"])
            scored["resolved_kingdom_name"] = str(resolved_query_kingdoms[rbp_id]["name"])
            ...
            if not gene_scores.empty:
                ...
                gene_views = build_gene_ranking_views(gene_scores)
                for ranking_name, view in gene_views.items():
                    all_gene_views[ranking_name].append(view)
                write_top_gene_lists(out_dir, rbp_id, gene_views["max"])
            if "label" in scored.columns:
                eval_rows.append(summarize_labeled_predictions(rbp_id, scored))
                ablation_rows.append(summarize_window_score_ablation(rbp_id, scored))
        ...
        if all_gene_rows:
            ...
            gene_ranking_cor = compute_gene_ranking_correlations(gene_scores)
            gene_ranking_cor.to_csv(out_dir / "gene_ranking_correlations.tsv", sep="\t", index=False)
            report["gene_ranking_correlations_tsv"] = str(out_dir / "gene_ranking_correlations.tsv")
            for ranking_name in GENE_RANKING_COLUMNS:
                if not all_gene_views[ranking_name]:
                    continue
                out_path = out_dir / f"gene_scores_by_{ranking_name}.tsv"
                pd.concat(all_gene_views[ranking_name], ignore_index=True).to_csv(out_path, sep="\t", index=False)
                report[f"gene_scores_by_{ranking_name}_tsv"] = str(out_path)
        if eval_rows:
        ...
        if ablation_rows:
            ...
            ablation_df.to_csv(out_dir / "window_score_ablation_metrics.tsv", sep="\t", index=False)
            report["window_score_ablation_metrics_tsv"] = str(out_dir / "window_score_ablation_metrics.tsv")
