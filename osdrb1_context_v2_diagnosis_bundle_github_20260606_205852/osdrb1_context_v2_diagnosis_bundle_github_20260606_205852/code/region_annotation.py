...
def load_region_model(gtf_path: str | Path) -> dict[str, dict]:
    ...
    for transcript_id, item in transcript_data.items():
        ...
        for exon_start, exon_end in exons:
            ...
            tx_pos += exon_len
        tx_length = tx_pos - 1
        explicit_5utr = sorted(tx_segments["5UTR"])
        explicit_3utr = sorted(tx_segments["3UTR"])
        inferred_segments = {"CDS": sorted(tx_segments["CDS"]), "5UTR": explicit_5utr.copy(), "3UTR": explicit_3utr.copy()}
        cds_segments = sorted(tx_segments["CDS"])
        if cds_segments:
            cds_start = min(seg[0] for seg in cds_segments)
            cds_end = max(seg[1] for seg in cds_segments)
            if not explicit_5utr and cds_start > 1:
                inferred_segments["5UTR"] = [(1, cds_start - 1)]
            if not explicit_3utr and cds_end < tx_length:
                inferred_segments["3UTR"] = [(cds_end + 1, tx_length)]
        model[transcript_id] = {
...
def _resolve_region_type(start: int, end: int, segments: dict[str, list[tuple[int, int]]], info: dict[str, object]) -> str:
...
def annotate_window_region(transcript_id: str, start: int, end: int, region_model: dict[str, dict]) -> dict[str, object]:
    ...
    if info is None:
        return {"region_type": "unknown", "inferred_region_type": "unknown"}
    return {
        "region_type": _resolve_region_type(start, end, info["segments"], info),
        "inferred_region_type": _resolve_region_type(start, end, info["inferred_segments"], info),
    }
...
def annotate_region_features(
    ...
    if not gtf_path or not Path(gtf_path).exists():
        ...
        frame["inferred_region_type"] = [item["inferred_region_type"] for item in annotated]
    if "inferred_region_type" not in frame.columns:
        frame["inferred_region_type"] = "NA"
    ...
    frame["inferred_is_CDS"] = (frame["inferred_region_type"] == "CDS").astype(float)
    frame["inferred_is_5UTR"] = (frame["inferred_region_type"] == "5UTR").astype(float)
    frame["inferred_is_3UTR"] = (frame["inferred_region_type"] == "3UTR").astype(float)
    frame["inferred_is_ncRNA"] = (frame["inferred_region_type"] == "ncRNA").astype(float)
    frame["inferred_is_unknown"] = (frame["inferred_region_type"].isin(["unknown", "NA"])).astype(float)
        ...
        rows.append(
            {
                ...
                "best_window_inferred_is_CDS": float(sub.iloc[0]["inferred_is_CDS"]),
                "best_window_inferred_is_5UTR": float(sub.iloc[0]["inferred_is_5UTR"]),
                "best_window_inferred_is_3UTR": float(sub.iloc[0]["inferred_is_3UTR"]),
                "best_window_inferred_is_ncRNA": float(sub.iloc[0]["inferred_is_ncRNA"]),
                "best_window_inferred_is_unknown": float(sub.iloc[0]["inferred_is_unknown"]),
                ...
                "top3_inferred_CDS_fraction": float(sub["inferred_is_CDS"].mean()),
                "top3_inferred_UTR_fraction": float((sub["inferred_is_5UTR"] + sub["inferred_is_3UTR"]).mean()),
                "top3_inferred_ncRNA_fraction": float(sub["inferred_is_ncRNA"].mean()),
                "top3_inferred_unknown_fraction": float(sub["inferred_is_unknown"].mean()),
            }
        )
