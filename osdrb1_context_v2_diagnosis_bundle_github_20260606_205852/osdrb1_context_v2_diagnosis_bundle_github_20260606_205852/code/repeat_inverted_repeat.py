...
RC_TABLE = str.maketrans({"A": "U", "U": "A", "C": "G", "G": "C"})
...
def reverse_complement(seq: str) -> str:
    return normalize_rna(seq).translate(RC_TABLE)[::-1]
...
def _compute_repeat_features_cached(seq: str) -> tuple[tuple[str, float | str], ...]:
    ...
    self_comp = float(np.nanmean([rc4, rc5, local_sc]))
    inv = float(np.nanmean([rc4, rc5, rc6, local_sc, min(longest / 12.0, 1.0), min(pair_count / 100.0, 1.0)]))
    simple_penalty = float(simple)
    return tuple(
        {
...
def compute_repeat_features(seq: str) -> Dict[str, float]:
    return dict(_compute_repeat_features_cached(normalize_rna(seq)))
