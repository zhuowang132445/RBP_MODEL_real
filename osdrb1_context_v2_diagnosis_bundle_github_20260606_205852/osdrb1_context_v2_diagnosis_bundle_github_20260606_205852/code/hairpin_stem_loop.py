...
def compute_paired_architecture_features(paired: np.ndarray) -> Dict[str, float]:
    ...
    if paired.size == 0:
        return {
            ...
            "paired_architecture_score": float("nan"),
            "is_high_paired_architecture_window": float("nan"),
            "MFE": float("nan"),
    ...
    paired_arch_score = float(
        np.nanmean(
            [
                ...
                local41,
                local81,
                min((max(runs) if runs else 0) / 50.0, 1.0),
                max(peakiness, 0.0),
            ]
        )
    )
    return {
        ...
        "paired_architecture_score": paired_arch_score,
        "is_high_paired_architecture_window": float(paired_arch_score >= 0.6),
        "MFE": float("nan"),
        "MFE_per_nt": float("nan"),
        "normalized_MFE": float("nan"),
        "predicted_stem_count": float("nan"),
...
def compute_hairpin_features(paired: np.ndarray) -> Dict[str, float]:
    return compute_paired_architecture_features(paired)
